...
def main():
    ...
    with open(file_name, "r") as file:
        for line in file:
            ...
            if (int(first_elve_lower_bound) <= int(second_elve_lower_bound)
            and int(first_elve_higher_bound) >= int(second_elve_higher_bound)):
                total_fully_contained_assigments += 1
            elif (int(first_elve_lower_bound) >= int(second_elve_lower_bound)
            and int(first_elve_higher_bound) <= int(second_elve_higher_bound)):
                total_fully_contained_assigments += 1
